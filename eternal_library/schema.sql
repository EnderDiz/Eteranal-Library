...
CREATE INDEX IF NOT EXISTS idx_books_status
    ON books(publication_status, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_books_title
    ON books(title COLLATE NOCASE);

CREATE TRIGGER IF NOT EXISTS trg_books_limit_10
BEFORE INSERT ON books
WHEN (SELECT COUNT(*) FROM books) >= 10
BEGIN
    SELECT RAISE(ABORT, 'library book limit reached');
END;
...
CREATE INDEX IF NOT EXISTS idx_book_chapters_book
    ON book_chapters(book_id, sort_order, id);
CREATE INDEX IF NOT EXISTS idx_book_chapters_publication
    ON book_chapters(book_id, publication_status, sort_order);
...
CREATE INDEX IF NOT EXISTS idx_library_activity_book
    ON library_activity(book_id, id DESC);
